...
function renderTarefa() {
    ...
    const statusLabels = { P: "Pendente", E: "Em andamento", C: "Concluida" };
...
async function quickStatusChange(sel) {
    ...
    if (!tarefa || newStatus === tarefa.status)
        return;
    sel.disabled = true;
    try {
...
function renderObservacoes() {
    ...
    el.querySelectorAll(".btn-del-obs").forEach((btn) => {
        btn.addEventListener("click", () => {
            if (confirm("Excluir esta observacao?"))
...
async function saveObsEdit(id, btn) {
    ...
    if (!texto) {
        alert("A observacao nao pode ficar vazia.");
        return;
    }
...
document.getElementById("form-edit-task")?.addEventListener("submit", async (e) => {
    ...
    if (!titulo || !descricao) {
        showAlert("alert-edit-task", "Preencha titulo e descricao.");
        return;
    }
    setLoading(btn, true, "Salvar");
    try {
