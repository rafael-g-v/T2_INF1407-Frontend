...
async function loadTarefas(silent = false) {
    ...
    if (silent && JSON.stringify(novas) === JSON.stringify(tarefas))
        return;
    tarefas = novas;
    renderTarefas();
}
...
async function quickStatusChange(tarefaId, novoStatus, sel) {
...
async function loadConvites(silent = false) {
    ...
    if (silent && JSON.stringify(novos) === JSON.stringify(convites))
        return;
    convites = novos;
    renderConvites();
}
function renderConvites() {
    ...
    const statusColor = { P: "badge-orange", A: "badge-green", R: "badge-red" };
...
function openEditTarefa(id) {
    ...
    document.getElementById("tarefa-responsavel").value = String(t.responsavel?.id ?? "");
...
async function confirmDeleteTarefa(id) {
    const t = tarefas.find((x) => x.id === id);
    if (!t || !confirm(`Excluir tarefa "${t.titulo}"?`))
        return;
    try {
